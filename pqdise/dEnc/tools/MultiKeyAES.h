...
namespace dEnc
{
	...
	public:
		...
		void ecbEncBlock(const block& plaintext, block* cyphertexts) const
		{
			...
			{
				for(int k = 0; k < 8; k++){
					dest[k] = plaintext ^ key[k].mRoundKey[0];
				}
				...
				{
					for(int k = 0; k < 8; k++){
						dest[k] = key[k].roundEnc(dest[k], key[k].mRoundKey[j]);
					}
				}

				for(int k = 0; k < 8; k++){
					dest[k] = key[k].finalEnc(dest[k], key[k].mRoundKey[10]);
				}
			...
			{
				// dest[0] = _mm_xor_si128(plaintext, key[0].mRoundKey[0]);
				dest[0] = plaintext ^ key[0].mRoundKey[0];

				for (u64 j = 1; j < 10; ++j)
				{
					// dest[0] = _mm_aesenc_si128(dest[0], key[0].mRoundKey[j]);
					dest[0] = key[0].roundEnc(dest[0], key[0].mRoundKey[j]);
				}
				// dest[0] = _mm_aesenclast_si128(dest[0], key[0].mRoundKey[10]);
				dest[0] = key[0].finalEnc(dest[0], key[0].mRoundKey[10]);
